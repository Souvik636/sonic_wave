...
class ZeroShotStrategy extends CategorizationStrategy {
  ...
  Future<List<AiProposedAlbum>> run(
    ...
    for (int i = 0; i < songReps.length; i += chunkSize) {
      ...
      final end = (i + chunkSize < songReps.length)
          ? i + chunkSize
          : songReps.length;
        ...
        for (final res in results) {
          ...
          if (listIndex >= 0 && listIndex < batchSongs.length) {
            ...
            if (category != 'UNASSIGNED' &&
                res.confidence >= ctx.request.zeroShotThreshold) {
              buckets
    ...
    for (final entry in buckets.entries) {
      ...
      final avgScore =
          scoredSongs.map((s) => s.score).reduce((a, b) => a + b) /
          scoredSongs.length;
      ...
      if (match != null) {
        proposals.add(
          AiProposedAlbum(
            ...
            description:
                'Songs classified into your custom category "$label" (merged with existing album).',
