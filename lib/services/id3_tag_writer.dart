...
class ID3TagWriter {
  ...
  static bool _hasCovrAtom(List<int> bytes) {
    ...
    while (true) {
      ...
      final dataAt = at + 8;
      if (dataAt + 4 <= bytes.length &&
          _matchesAt(bytes, dataAt, _dataMarker)) {
        return true;
      }
      from = at + 1;
    }
  ...
  static Future<bool> embedCoverArt({
    ...
    try {
      ...
      if (originalBytes.length >= 8 &&
          ...
          originalBytes[7] == 0x70) {
        // 'p'
        return await _embedM4aCoverArt(
          audioFile,
          originalBytes,
          imageBytes,
          mimeType,
      ...
      if (originalBytes[0] == 0x66 && // 'f'
          ...
          originalBytes[3] == 0x43) {
        // 'C'
        return await _embedFlacCoverArt(
          audioFile,
          originalBytes,
          imageBytes,
          mimeType,
      ...
      if (originalBytes.length >= 4) {
        final isWebm =
            originalBytes[0] == 0x1A &&
            originalBytes[1] == 0x45 &&
            originalBytes[2] == 0xDF &&
            originalBytes[3] == 0xA3;
        final isOgg =
            originalBytes[0] == 0x4F &&
            originalBytes[1] == 0x67 &&
            originalBytes[2] == 0x67 &&
            originalBytes[3] == 0x53;
        if (isWebm || isOgg) {
          debugPrint(
            '[ID3TagWriter] Skipping ID3 injection for WebM/OGG container.',
          );
          return false;
        }
      }
      ...
      return await _embedMp3CoverArt(
        audioFile,
        originalBytes,
        imageBytes,
        mimeType,
  ...
  static Future<bool> _embedMp3CoverArt(
    ...
    debugPrint(
      '[ID3TagWriter] Successfully embedded ID3v2.3 APIC cover art into MP3.',
    );
    return true;
  }
  ...
  static Future<bool> _embedFlacCoverArt(
    ...
    picPayload.addByte(0x00);
    picPayload.addByte(0x00);
    picPayload.addByte(0x03);
    // 32-bit uint MIME length
    final mimeLen = mimeAscii.length;
    picPayload.addByte((mimeLen >> 24) & 0xFF);
    picPayload.addByte((mimeLen >> 16) & 0xFF);
    picPayload.addByte((mimeLen >> 8) & 0xFF);
    picPayload.addByte(mimeLen & 0xFF);
    ...
    picPayload.addByte((imgLen >> 24) & 0xFF);
    picPayload.addByte((imgLen >> 16) & 0xFF);
    picPayload.addByte((imgLen >> 8) & 0xFF);
    picPayload.addByte(imgLen & 0xFF);
    ...
    await audioFile.writeAsBytes(builder.takeBytes(), flush: true);
    debugPrint(
      '[ID3TagWriter] Successfully embedded PICTURE metadata block into FLAC.',
    );
    return true;
  }
  ...
  static Future<bool> _embedM4aCoverArt(
    ...
    dataPayload.addByte((dataLen >> 24) & 0xFF);
    dataPayload.addByte((dataLen >> 16) & 0xFF);
    dataPayload.addByte((dataLen >> 8) & 0xFF);
    dataPayload.addByte(dataLen & 0xFF);
    dataPayload.add(ascii.encode('data'));
    dataPayload.addByte(0x00);
    dataPayload.addByte(0x00);
    dataPayload.addByte(0x00);
    dataPayload.addByte(dataFlags);
    dataPayload.addByte(0x00);
    dataPayload.addByte(0x00);
    dataPayload.addByte(0x00);
    dataPayload.addByte(0x00);
    ...
    covrPayload.addByte((covrLen >> 24) & 0xFF);
    covrPayload.addByte((covrLen >> 16) & 0xFF);
    covrPayload.addByte((covrLen >> 8) & 0xFF);
    covrPayload.addByte(covrLen & 0xFF);
    ...
    final udtaAtom = _findAtom(
      originalBytes,
      moovAtom.payloadOffset,
      moovAtom.offset + moovAtom.length,
      'udta',
    );
    final builder = BytesBuilder(copy: false);

    if (udtaAtom != null) {
      final metaAtom = _findAtom(
        originalBytes,
        udtaAtom.payloadOffset,
        udtaAtom.offset + udtaAtom.length,
        'meta',
      );
      if (metaAtom != null) {
        final metaStart = metaAtom.payloadOffset + 4;
        final ilstAtom = _findAtom(
          originalBytes,
          metaStart,
          metaAtom.offset + metaAtom.length,
          'ilst',
        );
        if (ilstAtom != null) {
          final existingCovr = _findAtom(
            originalBytes,
            ilstAtom.payloadOffset,
            ilstAtom.offset + ilstAtom.length,
            'covr',
          );
          int delta = 0;
          if (existingCovr != null) {
            ...
            builder.add(
              Uint8List.sublistView(originalBytes, 0, existingCovr.offset),
            );
            builder.add(covrAtomBytes);
            builder.add(
              Uint8List.sublistView(
                originalBytes,
                existingCovr.offset + existingCovr.length,
              ),
            );
          } else {
            ...
            builder.add(
              Uint8List.sublistView(
                originalBytes,
                0,
                ilstAtom.payloadOffset + ilstAtom.payloadLength,
              ),
            );
            builder.add(covrAtomBytes);
            builder.add(
          ...
          _adjustChunkOffsets(
            newBytes,
            moovAtom.offset,
            moovAtom.length + delta,
            delta,
          );
          await audioFile.writeAsBytes(newBytes, flush: true);
          debugPrint(
            '[ID3TagWriter] Successfully embedded covr atom into M4A ilst metadata.',
          );
          return true;
        }
      }
    ...
    builder.add(
      Uint8List.sublistView(
        originalBytes,
        0,
        moovAtom.payloadOffset + moovAtom.payloadLength,
      ),
    );
    ...
    await audioFile.writeAsBytes(newBytes, flush: true);
    debugPrint(
      '[ID3TagWriter] Successfully constructed udta/meta/ilst/covr in M4A.',
    );
    return true;
  }

  // --- MP4 Atom Helpers ---
  static _Atom? _findAtom(
    Uint8List bytes,
    int start,
    int end,
    String targetType,
    ...
    while (pos + 8 <= end && pos < bytes.length) {
      ...
      final type = String.fromCharCodes(
        Uint8List.sublistView(bytes, pos + 4, pos + 8),
      );
      if (type == targetType) {
  ...
  static void _adjustChunkOffsets(
    Uint8List bytes,
    int moovOffset,
    int moovLen,
    int delta,
    ...
    while (pos + 16 <= moovEnd && pos + 16 <= bytes.length) {
      if (bytes[pos + 4] == 0x73 &&
        ...
        for (int i = 0; i < count; i++) {
          ...
          final current =
              (bytes[entryOffset] << 24) |
              (bytes[entryOffset + 1] << 16) |
              (bytes[entryOffset + 2] << 8) |
              bytes[entryOffset + 3];
